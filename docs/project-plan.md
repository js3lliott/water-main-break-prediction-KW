...
### Phase 8 — Write-up (≈1–2 days) — *done*

README rewritten around the result. It opens with the network map, the headline
number, and the survivorship caveat — a number, a map, and a limitation, which
was the stated bar. Every figure in it is cross-checked against the warehouse
programmatically rather than transcribed.

The old README described a pipeline that no longer exists: it linked to an
`nbs/` directory that was never in this repo, cited 2,752 observations and 46
features against an actual 2,925 and 37, and carried a table of contents listing
seven sections that were never written.

`docs/model-card.md` gained the phase 7 finding (`WATMAINID` losing uniqueness)
and a fuller "not yet done" — the rate and survival models, point-in-time
attributes, and the features most likely to change the conclusion.

`analysis/figures.py` gained `08_risk_map`, the network drawn as centrelines
coloured by risk. It is generated by `make figures` rather than screenshotted,
so the README hero stays true as the data refreshes.
